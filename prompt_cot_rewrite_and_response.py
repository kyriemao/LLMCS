...
from promptor import CoTRewriteAndResponsePromptor
...
def main():
    ...
    promptor = CoTRewriteAndResponsePromptor()
        ...
        for i in trange(len(test_dialogs)):
            ...
            for j in trange(len(turns)):
                ...
                if sample_id in finished_samples:
                    ...
                    response_list = finished_samples[sample_id]['predicted_response']
                    ...
                    response_list = [turns[j]['response']] * args.n_completion
                else:
                    while True:
                        try:
                            n_outputs = generator.generate(prompt, promptor.parse_returned_text)
                            rewrite_list, response_list, cot_list, rewrite_part_text_list = list(zip(*n_outputs))
                ...
                # output info
                print("{}, sample_id: {} \n predicted rewrite: {} \n cot: {} \n predicted response: {}".format(args.work_dir, sample_id, rewrite_list[0], cot_list[0], response_list[0]))
